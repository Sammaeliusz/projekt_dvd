...
with open("/projekt/back/data/menubar.txt", 'rb') as f:
    menubar = f.read().decode("utf-8")

#''.join(choice(ascii_letters) for i in range(32)
app.addroute(Create.flask.route(app, '/', Create.addWrapper(index, Create.mainFunction.fromTemplate("index.html", menubar=menubar)), "index"))
